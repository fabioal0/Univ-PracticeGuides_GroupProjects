/*******************************************************************************
 ...
 Nome : Fábio Daniel Ribeiro Alves                                  NMEC : 84734
...
{ /* função recursiva - insira o seu codigo */
    ...
    if(aux->PtLeft == NULL && aux->PtRight==NULL){

        *proot = NULL;
        return;
    }
    
    else if(aux->PtLeft == NULL) {
    ...
    return ABPDeleteMin(&aux->PtLeft);
}

void ABPDeleteMax (PtABPNode *proot)
{ /* função repetitiva - insira o seu codigo */  
    ...
    if(aux->PtRight == NULL && aux->PtLeft==NULL){

        *proot = NULL;
        return;
    }    
    
    else if(aux->PtRight == NULL) {
    ...
    return ABPDeleteMax(&aux->PtRight);   
}
...
{ /* função recursiva - insira o seu codigo */ 
    ...
    if(proot->Elem == pvalue) {            
        ...
        if(proot->PtLeft == NULL || proot->PtLeft->PtRight == NULL) { 
            Error = OK;
            return proot;  
        }
        
        else if (proot->PtLeft->PtRight->Elem > pvalue){
            return ABPCeilValue(proot->PtLeft,pvalue);        
        }
...
{ /* função recursiva - insira o seu codigo */ 
    ...
    if(proot->Elem >= pvalue) {
        unsigned int sum = ABPRank(proot->PtLeft,pvalue);
        Error = OK; 
        return sum; 
    }
    
    else{
        unsigned int sum = 1 + ABPSize(proot->PtLeft) + ABPRank(proot->PtRight,pvalue);
        Error = OK; 
        return sum; 
    }
    
    Error = OK; 
    return 0; //para evitar warnings no ubunto 
...
{ /* função recursiva - insira o seu codigo */ 
    ...
    if(proot->Elem > phigh) {
        ...
        if(sizeof (proot->Elem)>sizeof (int)) { 
            Error = NO_MEM; 
            return ; 
        }
...
{ /* função repetitiva - insira o seu codigo */ 
    ...
    PtStack aux = StackCreate(sizeof(PtABPNode));
...
/*******************************************************************************
  Função repetitva que pesquisa o no minimo que se encontra na arvore a
