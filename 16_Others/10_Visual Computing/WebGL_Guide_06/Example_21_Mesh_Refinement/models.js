...
function recSubdivisionCentroid(v1, v2, v3,
    ...
    if (recursionDepth == 0) {
        ...
        var mid1 = computeCentroid(v1, v2, v3);

        // Colors are also averaged

        var co1 = computeCentroid(c1, c2, c3);

        // 3 recursive calls

        recSubdivisionCentroid(v1, mid1, v2, c1, co1, c2,
            coordsArray, colorsArray, recursionDepth - 1);

        recSubdivisionCentroid(v3, v2, mid1, c3, c2, co1,
            coordsArray, colorsArray, recursionDepth - 1);

        recSubdivisionCentroid(v1, v3, mid1, c1, c3, co1,
            coordsArray, colorsArray, recursionDepth - 1);
...
function moveToSphericalSurface(coordsArray) {
    ...
    var v = coordsArray.slice(0,3)

    for (i = 0; i < coordsArray.length / 3; i += 3) {
        v = coordsArray.slice(i,i+2)
        normalize(v)

        coordsArray[i] = v[i]
        coordsArray[i + 1] = v[i + 1]
        coordsArray[i + 2] = v[i + 2]
    }
