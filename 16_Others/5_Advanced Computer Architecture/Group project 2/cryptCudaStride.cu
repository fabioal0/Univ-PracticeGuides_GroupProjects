//
// Tomás Oliveira e Silva / António Rui Borges
//
// ACA 2019/2020
//
...
__global__ static void modify_sector_cuda_kernel (unsigned int * __restrict__ sector_data, unsigned int * __restrict__ sector_number,
                                                  unsigned int n_sectors, unsigned int sector_size, unsigned int stride);
...
int main (int argc, char **argv)
{
  ...
  if ((sector_data_size + sector_number_size) > (size_t) 1.3e9)
     { fprintf (stderr,"The GTX 1060 cannot handle more than 5.5GiB of memory!\n");
       exit (1);
     }
  ...
  stride = 2048;                                                       // optimize!
  blockDimX = 64;                                                    // optimize!
  blockDimY = 8;                                                    // optimize!
  blockDimZ = 1;                                                    // optimize!
  gridDimX = 1 << 2;                                               // optimize!
  gridDimY = 1 << 10;                                                // optimize!
  gridDimZ = 1;                                                     // optimize!
  if ((blockDimX * blockDimY * blockDimZ * gridDimX * gridDimY * gridDimZ) != N_SECTORS)
  ...
  (void) get_delta_time ();
  modify_sector_cuda_kernel <<<grid, block>>> (device_sector_data, device_sector_number, n_sectors, sector_size, stride);
  ...
  (void) get_delta_time ();
  unsigned int n_words = sector_size / 4u;
  for (i = 0; i < N_SECTORS; i++)
    modify_sector_cpu_kernel (&host_sector_data[i%stride+(i/stride)*n_words*stride],host_sector_number[i], n_sectors, sector_size, stride);
  printf("The cpu kernel took %.3e seconds to run (single core)\n",get_delta_time ());

  // compare
  for(i = 0; i < (int) sector_data_size / (int) sizeof (unsigned int); i++)
  { // printf ("%u = %u\n", host_sector_data[i], modified_device_sector_data[i]);
...
static void modify_sector_cpu_kernel (unsigned int *sector_data, unsigned int sector_number, unsigned int n_sectors,
                                      unsigned int sector_size, unsigned int stride)
{
  unsigned int x, i, a, c, n_words, tspace;
 ...
 // modify the sector data
  tspace = n_sectors * sector_size;
  for (i = 0u; i < n_words; i++)
  { x = a * x + c;                            // update the pseudo-random generator state
    sector_data[(i*stride)%tspace] ^= x;                      // modify the sector data
  }
}
...
__global__ static void modify_sector_cuda_kernel (unsigned int * __restrict__ sector_data, unsigned int * __restrict__ sector_number,
                                           unsigned int n_sectors, unsigned int sector_size, unsigned int stride)
{
  unsigned int x, y, idx, i, a, c, n_words, tspace;
  ...
  // adjust pointers
  sector_data += (idx % stride + (idx / stride) * n_words * stride);
  sector_number += idx;
