class BayesNet:
    ...
    def individualProb(self, var, val):
        ...
        return sum([self.jointProb(c + [(var, val)]) for c in self._gen_conjunction(variaveis)])

    def _gen_conjunction(self, variaveis):

        if len(variaveis) == 1:
            return [[(variaveis[0], True)], [(variaveis[0], False)]]

        l = []
        for c in self._gen_conjunction(variaveis[1:]):
