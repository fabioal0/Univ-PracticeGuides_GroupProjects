...
int Destroy(PtDigraph *pdig) {
    PtDigraph TmpDigraph = *pdig;
    PtBiNode Vertex, Edge;
...
int InVertex(PtDigraph pdig, unsigned int pv) {
    ...
    if (pdig->Head == NULL || pdig->Head->Number > pv) { /* inserção à cabeça do dígrafo */
        Node->PtNext = pdig->Head;
        pdig->Head = Node;
        if (Node->PtNext != NULL) Node->PtNext->PtPrev = Node;
    } else { /* inserção à frente do nó de inserção */
        if ((Insert = InPosition(pdig->Head, pv)) == NULL) { /* inserção sem sucesso, porque o vértice já existe */
            DestroyBiNode(&Node);
            return REP_VERTEX;
        }
...
int OutVertex(PtDigraph pdig, unsigned int pv) {
    ...
    while (Delete->PtAdj != NULL) /* remover a lista de adjacências */ { /* atualizar semigrau incidente da cabeça da lista das arestas */
    ...
    /* remoção do vértice */
    if (Delete == pdig->Head) { /* remoção do vértice da cabeça do dígrafo */
        if (Delete->PtNext != NULL) Delete->PtNext->PtPrev = NULL;
        pdig->Head = Delete->PtNext;
    } else { /* remoção de outro vértice do dígrafo */
    ...
    while (Vertex != NULL) { /* remover a aresta deste vértice para o vértice removido */
        if ((Edge = OutPosition(Vertex->PtAdj, pv)) != NULL) {
            if (Edge == Vertex->PtAdj) { /* remoção da aresta da cabeça da lista das arestas */
                if (Edge->PtNext != NULL) Edge->PtNext->PtPrev = NULL;
                Vertex->PtAdj = Vertex->PtAdj->PtNext;
            } else { /* remoção de outra aresta da lista das arestas */
...
int InEdge(PtDigraph pdig, unsigned int pv1, unsigned int pv2, int pcost) {
    ...
    if (!pdig->Type) /* se é grafo, inserir também a aresta v2-v1 */
        if (InsertEdge(V2, V1, pcost) != OK) { /* se a aresta v2-v1 não foi inserida, remover a aresta v1-v2 */
            DeleteEdge(V1, V2);
            return NO_MEM;
        }
...
PtDigraph CreateFile(char *pfilename) {
    PtDigraph Digraph;
    FILE *PtF;
    unsigned int Type, NVertexes, NEdges, V, E, Vert1, Vert2;
    int Status, Cost;
    ...
    fscanf(PtF, "%d %d", &NVertexes, &NEdges);
    fscanf(PtF, "%*c");

    if ((Digraph = Create(Type)) == NULL) {
        fclose(PtF);
        return NULL;
    }
    ...
    for (E = 0; E < NEdges; E++) {
        Status = fscanf(PtF, "%d %d %d", &Vert1, &Vert2, &Cost);
        fscanf(PtF, "%*c");
        if (Status != 3) {
            Destroy(&Digraph);
            fclose(PtF);
...
int VertexType(PtDigraph pdig, unsigned int pv) {
    if (pdig == NULL) {
        return NO_DIGRAPH;
    } else if (pdig->Head == NULL) {
        return DIGRAPH_EMPTY;
    }

    PtBiNode aux = pdig->Head;

    for (int i = 0; i <= pdig->NVertexes; i++) {

        if (aux->Number == pv) {

            PtVertex aux1 = aux->PtElem;

            if (aux1->InDeg == 0 && aux1->OutDeg == 0) {
                return DISC;
            } else if (aux1->InDeg == 0 && aux1->OutDeg != 0) {
                return SOURCE;
            } else if (aux1->InDeg != 0 && aux1->OutDeg == 0) {
                return SINK;
            } else {
...
int VertexOutDegreeCentrality(PtDigraph pdig, unsigned int pv, double *pcent) {
    ...
    for (int i = 0; i < pdig->NVertexes; i++) {
        ...
        if (aux->Number == pv) {
            ...
            aux1 = (PtVertex) aux->PtElem;
            *pcent = (double) aux1->OutDeg / pdig->NVertexes;
            return OK;
...
int AllPredecessors(PtDigraph pdig, unsigned int pv, PtQueue *pqueue) { //consultar o indegre  0 nao tem predecessores e n-1 todos sao predecessores
    /* insira o seu codigo */
    return OK;
}
...
//
//            while (tmpBehindNode->Number < tmpAdjNode->Number) {
//
//                while (tmpBehindNode->Number < n + 1) {
//                    tmpBehindNode = tmpBehindNode->PtNext;
//                }
...
static int InsertEdge(PtBiNode pv1, PtBiNode pv2, int pcost) {
    ...
    if (pv1->PtAdj == NULL || pv1->PtAdj->Number > pv2->Number) { /* inserção à cabeça da lista das arestas */
        Node->PtNext = pv1->PtAdj;
        pv1->PtAdj = Node;
        if (Node->PtNext != NULL) Node->PtNext->PtPrev = Node;
    } else { /* inserção à frente do nó de inserção */
...
static void DeleteEdge(PtBiNode pv1, PtBiNode pv2) {
    ...
    if (Delete == pv1->PtAdj) /* remoção da aresta */ { /* remoção da aresta da cabeça da lista das arestas */
        if (Delete->PtNext != NULL) Delete->PtNext->PtPrev = NULL;
        pv1->PtAdj = Delete->PtNext;
    } else { /* remoção de outra aresta do vértice */
        Delete->PtPrev->PtNext = Delete->PtNext;
        if (Delete->PtNext != NULL) Delete->PtNext->PtPrev = Delete->PtPrev;
    }
